...
Two different icon languages, and the difference is not decorative:

  Control icons - toolbar buttons, tools, arrows - are 16x16 and monochrome,
  drawn white on transparent and tinted at draw time, so one sheet serves
  normal, dimmed, hovered and selected states and a theme swap recolours all of
  them at once.

  They carry two tones rather than one. Aseprite's tool icons are not flat
  silhouettes - the pencil has a lit body and a dark tip, the bucket a lit face
  and a shaded side - and a silhouette throws that away. The second tone is
  encoded as half alpha in the same sheet: tinting multiplies, so `+` comes out
  as the tint at 50% over whatever is behind it, which is exactly a shade. One
  sheet, one draw call, no second colour to thread through the theme.

  File icons - folder, document, cartridge - are full-colour 16x16 art with a
  1px outline and two fill tones, lifted pixel for pixel from Picotron's own
  icon browser. These are pictures, not symbols; tinting one would destroy it.

The size matters as much as the style. Aseprite's control icons are 16x16 on a
960x540 framebuffer - a quarter of the density Picotron's 7x7 icons had on
480x270 - so this sheet is not the Picotron sheet enlarged, it is drawn for a
different grid.
"""
import os
import sys
...
# --- control icons: 16x16, two tones, tinted at draw time --------------------
#
# `#` is full ink, `+` is the same tint at half alpha (a shade), `.` is clear.
# Eight per row, in the order Studio binds them.
ICON_CELL = 16
ICONS = {
    # Drawing tools, drawn in Aseprite's idiom: a lit body, a shaded edge, and
    # enough internal space that the shape still reads at 16px.
    "pencil": [
        "..........####..", ".........##++##.", "........##++++#.",
        ".......##++++##.", "......##++++##..", ".....##++++##...",
        "....##++++##....", "...##++++##.....", "..##++++##......",
...
def main():
    ...
    # `+` is the same white at half alpha. Tinting multiplies, so it lands as
    # the tint at 50% over the ground - a shade, from one sheet.
    icons = sheet(ICONS, ICON_CELL, 8, lambda mark: (255, 255, 255, 128 if mark == "+" else 255))
